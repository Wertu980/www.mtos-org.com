// server.js — MTOS Auth Backend using Neon PostgreSQL (fixed)
...
const { randomUUID } = require("crypto");        // ✅ use UUIDs
...
// ---- PostgreSQL Connection (Neon requires SSL) ----
const pool = new Pool({
...
app.post("/v1/auth/signup", requireApiKey, signupRules, async (req, res) => {
  ...
  try {
    ...
    const userId = randomUUID(); // ✅ valid UUID for Neon
...
app.listen(PORT, HOST, () => {
  console.log(`✅ MTOS Auth Server running at http://${HOST}:${PORT}`);
